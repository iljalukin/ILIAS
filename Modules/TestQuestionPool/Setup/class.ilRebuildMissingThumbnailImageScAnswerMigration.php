...
class ilRebuildMissingThumbnailImageScAnswerMigration implements Setup\Migration
{
    ...
    {
        return 'Rebuild Missing Thumbnail Images for Single Choice Question Answers';
    }
    ...
    public function step(Environment $environment): void
    {
        ...
        $query = 'select qst.question_fi, qst.thumb_size, a.imagefile from qpl_qst_sc qst inner join qpl_a_sc a on qst.question_fi=a.question_fi where qst.thumb_size is null and LENGTH(TRIM(a.imagefile)) > 0 group by qst.question_fi limit 1';
        $result = $this->db->query($query);
        while ($question = $this->db->fetchAssoc($result)) {
            $question_obj = assSingleChoice::instantiateQuestion((int) $question['question_fi']);
    ...
    {
        $query = 'select count(distinct qst.question_fi) as cnt from qpl_qst_sc qst inner join qpl_a_sc a on qst.question_fi=a.question_fi where qst.thumb_size is null and LENGTH(TRIM(a.imagefile)) > 0';
        $result = $this->db->query($query);
        ...
        if ($question->isSingleLine()) {
            foreach ($question->getAnswers() as $answer) {
                if ($answer->getImage()) {
                    ...
                    $this->log("\nRebuilt thumbnail for image " . $answer->getImage() . " of answer order " . $answer->getOrder() . " with width " . $width . " for question " . $question->getId());
                    if ($width > $imageWidth) {
                        $imageWidth = $width;
                    }
                }
